...
def is_mail(string: str) -> str:
    ...
    if re.match(r"^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$", string) is None:
        raise HTTPException(status_code=422, detail=CommonErrorMessages.MAIL_INVALID)
    return string
...
def is_password(string: str | None) -> str | None:
    ...
    if re.match(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$", string) is None:
        raise HTTPException(status_code=422, detail=CommonErrorMessages.PASSWORD_NOT_SECURE)
    return string
...
def is_name(string: str) -> str:
    ...
    if re.match(r"^[\p{L}\s]+$", string) is None:
        raise HTTPException(status_code=422, detail=CommonErrorMessages.NAME_INVALID)
    return string
...
def is_login(string: str) -> str:
    ...
    if re.match(r"^[a-zA-Z0-9_]+$", string) is None:
        raise HTTPException(status_code=422, detail=CommonErrorMessages.LOGIN_INVALID)
    return string
