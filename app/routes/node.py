...
async def get_root_node(academic_year: int,
                        current_account: AuthenticatedAccount) -> PydanticNodeModelWithChildIds:
    """
...
async def get_node_by_id(academic_year: int,
    ...
    """
    return await NodeService.get_node_by_id(academic_year, node_id, current_account)

@nodeRouter.get("/root/arborescence", status_code=200, response_model=PydanticNodeModel)
async def get_arborescence_from_root(academic_year: int,
                                     current_account: AuthenticatedAccount) -> PydanticNodeModel:
    """
...
async def get_arborescence_from_node(academic_year: int,
                                     node_id: int, 
                                     current_account: AuthenticatedAccount) -> PydanticNodeModel:
    """
...
async def add_node(academic_year: int,
...
async def modify_node(academic_year: int,
                      node_id: int,
