...
async def get_root(academic_year: int) -> NodeInDB:
    ...
    Args:
        academic_year (int): The academic year to search the root node from.
    Returns:
        NodeInDB : The root node of the given academic year.
    """
...
async def get_node_map(academic_year: int) -> dict[int, NodeInDB]:
    ...
    CAREFUL ! Does not check for permissions !
    Args:
        academic_year (int): The academic year to search the nodes from.
    Returns:
        dict[int, NodeInDB] : A dict that associates nodes to their ids.
    """
    all_nodes : list[NodeInDB] = await NodeInDB.filter(academic_year=academic_year)\
                                               .prefetch_related("child_nodes")
    return {node.id: node for node in all_nodes}

async def add_ues_to_node_model(node: PydanticNodeModel | PydanticNodeModelWithChildIds,
                                academic_year: int) -> PydanticNodeModel | PydanticNodeModelWithChildIds:
    """
    This method adds the UEs to the given node model.
    CAREFUL ! Does not check for permissions.
    Args:
        node (PydanticNodeModel | PydanticNodeModelWithChildIds): The node to add the UEs to.
        academic_year (int): The academic year to search the UEs from.
    Returns:
        PydanticNodeModel | PydanticNodeModelWithChildIds : The node with the UEs added.
    """
...
async def build_tree_recursive(current_node: NodeInDB,
                               node_map: dict[int, NodeInDB]) -> PydanticNodeModel:
    """
    ...
    return cast(PydanticNodeModel,
                await add_ues_to_node_model(pydantic_node,
                                            current_node.academic_year))

async def build_node_with_child_id(node: NodeInDB,
                                   children: Optional[
                                       list[NodeInDB]
                                    ] = None) -> PydanticNodeModelWithChildIds:
    """
...
async def get_node_by_id(academic_year: int,
                         node_id: int,
                         current_account: AccountInDB) -> PydanticNodeModelWithChildIds:
    """
    This method returns the Node of the given node id.
    Also returns the contents of the sub-nodes.
    """
    await check_permissions(AvailableServices.NODE_SERVICE,
...
async def get_root_node(academic_year: int,
                        current_account: AccountInDB) -> PydanticNodeModelWithChildIds:
    """
...
async def get_root_arborescence(academic_year: int,
                                current_account: AccountInDB) -> PydanticNodeModel:
    """
...
async def get_all_child_nodes(academic_year: int,
                              node: int | NodeInDB,
                              current_account: AccountInDB | None = None) -> PydanticNodeModel:
    """
...
async def create_node(academic_year: int,
                      node_to_add: PydanticNodeCreateModel,
                      current_account: AccountInDB) -> PydanticNodeModelWithChildIds:
    """
    ...
    if (node_to_add.parent_id is not None
        and await UEInDB.exists(parent=node_to_add.parent_id, academic_year=academic_year)):
        raise HTTPException(status_code=400,
                            detail=CommonErrorMessages.FOLDER_AND_UE_NOT_ENABLED.value)

    parent: NodeInDB | None = await NodeInDB.get_or_none(id=node_to_add.parent_id)
    if parent is None:
        raise HTTPException(status_code=404,
                            detail=CommonErrorMessages.NODE_NOT_FOUND.value)
...
async def update_node(academic_year: int,
                      node_id: int,
                      new_data: PydanticNodeUpdateModel,
    ...
    node_to_update: NodeInDB | None = await NodeInDB.get_or_none(id=node_id,
                                                                 academic_year=academic_year)
    ...
    if (new_data.parent_id is not None
        and not await NodeInDB.exists(id=new_data.parent_id, academic_year=academic_year)):
        raise HTTPException(status_code=404,
                            detail=CommonErrorMessages.NODE_NOT_FOUND.value)

    # We check if the parent Node does not already contain a children that are UES and not Nodes.
    if (new_data.parent_id is not None
        and await UEInDB.exists(parent=new_data.parent_id, academic_year=academic_year)):
        raise HTTPException(status_code=400,
                            detail=CommonErrorMessages.FOLDER_AND_UE_NOT_ENABLED.value)

    if new_data.parent_id is not None:
        parent: NodeInDB | None = await NodeInDB.get_or_none(id=new_data.parent_id)
        if parent is None:
            raise HTTPException(status_code=404,
                                detail=CommonErrorMessages.NODE_NOT_FOUND.value)
        node_to_update.parent = parent
...
async def delete_node(academic_year: int,
                      node_id: int,
                      current_account: AccountInDB) -> None:
    """
    ...
    node_to_delete: NodeInDB | None = await NodeInDB.get_or_none(id=node_id,
                                                                 academic_year=academic_year)\
                                                    .prefetch_related("child_nodes")
