"""
This module contains the service methods for the Course model.
"""
...
async def get_course_by_id(academic_year: int,
                           course_id: int,
                           current_account: AuthenticatedAccount) -> PydanticCourseModel:
    """
    ...
    course: CourseInDB | None = await CourseInDB.get_or_none(id=course_id)\
                                                .prefetch_related("course_type")

    if course is None:
        raise HTTPException(status_code=404,
                            detail=CommonErrorMessages.COURSE_NOT_FOUND.value)
...
async def add_course(academic_year: int,
                    body: PydanticCreateCourseModel,
                    current_account: AuthenticatedAccount) -> PydanticCourseModel:
    """
    ...
    if body.group_count < 0:
        raise HTTPException(status_code=422,
                            detail=CommonErrorMessages.GROUP_VALUE_INCORRECT.value)

    course_type: CourseTypeInDB | None = await CourseTypeInDB.get_or_none(id=body.course_type_id)

    if course_type is None:
        raise HTTPException(status_code=404,
                            detail=CommonErrorMessages.COURSE_TYPE_NOT_FOUND.value)

    course_type_pydantic = PydanticCourseTypeModel.model_validate(course_type)

    course_to_create: CourseInDB = CourseInDB(academic_year=body.academic_year,
...
async def modify_course(academic_year: int,
                        course_id: int, body: PydanticModifyCourseModel,
    ...
    if body.duration is not None:
        if body.duration < 0:
            raise HTTPException(status_code=422,
                                detail=CommonErrorMessages.DURATION_VALUE_INCORRECT.value)
...
async def delete_course(academic_year: int,
                        course_id: int,
                        current_account: AuthenticatedAccount) -> None:
    """
